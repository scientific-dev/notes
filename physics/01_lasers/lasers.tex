...
So, we have to make the system to undergo non equilibrium conditions by supplying energy to the system (uniformly for better output) from an external source. This process is called as \textbf{Pumping}. \vspace{.2cm}
...
There are certain exceptional energy states in certain elements where the average time taken for an atom to remain in those energy states are as high as $10^{-3}s$ which is large enough (compared to $10^{-8}s$) for photons to excite it through stimulation.
...
% \textbf{Characterstics of Lasers:}

% \begin{itemize}
    % \item \textbf{Directionality:} As all these photons travel in the same direction, it results in a highly collimated beam of light.
    % \item \textbf{Monochromaticity:} The photon emitted and photon which stimulates will be of same energy, i.e. $\Delta E$ between the excited and the ground state. As the energy will be same, so will be its frequency and wavelength $(\Delta E = h\nu = hc/\lambda)$ resulting in highly collimated monochromatic beam of light. 
% \end{itemize}
